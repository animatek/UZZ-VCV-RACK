...
- The slider is also the **meter**, and the number of bars follows what there is to show: one for a mono path, two as soon as both IN L and IN R are patched (whatever the polyphony — two mono cables are still two paths), and one per channel, up to 16, under per-channel envelopes. Each bar draws the gain applied to its channel, not the audio level: metering the signal would turn it into an output VU and the ducking would stop being legible. Brightness follows the envelope, so a duck reads twice — shorter and dimmer. Drawn in Rack's light layer with a soft bloom, so it stays lit when you dim the room brightness.
