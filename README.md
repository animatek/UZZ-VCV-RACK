...
- The slider is also the **meter**, split into two bars for left and right. Each draws the gain applied to its side, not the audio level: metering the signal would turn it into an output VU and the ducking would stop being legible. With the shared envelope both bars move together; they part company under per-channel envelopes. Brightness follows the envelope, so a duck reads twice — shorter and dimmer. Drawn in Rack's light layer with a soft bloom, so it stays lit when you dim the room brightness.
