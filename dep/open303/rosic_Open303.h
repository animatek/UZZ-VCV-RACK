...
namespace rosic
{
  ...
  */
  ...
  public:
    ...
    /** ATEK303: gancho para sustituir el oscilador por uno propio sin tocar el
    resto del motor. Si externalOscillator es NULL se usa el oscilador de tablas
    original de Open303. Ver dep/open303/PROVENANCE.md. */
    struct ExternalOscillator
    {
      virtual ~ExternalOscillator() {}
      virtual void setFrequency(double hz, double sampleRate) = 0;
      virtual void resetPhase() = 0;
      virtual double getSample() = 0;
    };
    ExternalOscillator* externalOscillator = NULL;

    /** ATEK303: mismo mecanismo para el filtro. Con NULL se usa el TeeBeeFilter
    original. Ver dep/open303/PROVENANCE.md. */
    struct ExternalFilter
    {
      virtual ~ExternalFilter() {}
      virtual void setSampleRate(double sampleRate) = 0;   // ya sobremuestreado
      virtual void setCutoff(double hz) = 0;
      virtual void setResonance(double percent) = 0;
      virtual void reset() = 0;
      virtual double getSample(double in) = 0;
    };
    ExternalFilter* externalFilter = NULL;

    /** ATEK303: margen lineal del OTA BA662A. El VCA real es un OTA y su entrada
    diferencial deja de ser lineal por encima de cierto nivel; Open303 multiplica
    limpio. Con 0 se desactiva. Ojo: la señal que entra aquí NO cambia con el acento
    (medido) — el acento sube la ganancia, no la entrada — así que esto aporta color
    según lo fuerte que empuje el filtro, no más ladrido en los acentos. */
    double otaHeadroom = 0.0;

    /** ATEK303: si es true, el slide se hace sobre el *logaritmo* de la frecuencia,
    que es lo que hace el hardware — la red R91·C35 desliza la tensión de control y el
    conversor exponencial la convierte después, así que el glide es constante en
    semitonos/segundo. Con false se mantiene el comportamiento original de Open303
    (deslizar los Hz linealmente). Ver dep/open303/PROVENANCE.md. */
    bool slideInPitchDomain = false;

    /** ATEK303: cambia el dominio del slide convirtiendo el estado que el limitador
    tiene guardado. Sin esta conversión, un estado escrito en hercios se reinterpreta
    como logaritmo (o al revés) y el oscilador recibe una frecuencia absurda. */
    void setSlideInPitchDomain(bool inPitchDomain)
    {
      if( inPitchDomain == slideInPitchDomain )
        return;
      slideInPitchDomain = inPitchDomain;
      pitchSlewLimiter.setState(inPitchDomain ? log(oscFreq) : oscFreq);
    }

    /** ATEK303: fija directamente la constante de tiempo del slide en ms. setSlideTime()
    aplica un factor 0,2 que el propio autor marca como pendiente de ajustar, y el
    constructor no lo usa; esto evita esa ambigüedad. */
    void setSlideTimeConstant(double ms) { pitchSlewLimiter.setTimeConstant((float) ms); }
    ...
    void setResonance(double newResonance)                                  // ATEK303
    {
      if( externalFilter != NULL )
        externalFilter->setResonance(newResonance);
  ...
  inline double Open303::getSample()
  {
    ...
    double instFreq;                                                      // ATEK303
    if( slideInPitchDomain )
      instFreq = exp(pitchSlewLimiter.getSample(log(oscFreq)));
    else
      instFreq = pitchSlewLimiter.getSample(oscFreq);
    if( externalOscillator != NULL )                                  // ATEK303
      externalOscillator->setFrequency(instFreq*pitchWheelFactor, oversampling*sampleRate);
    else
    {
    ...
    if( externalFilter != NULL )                    // ATEK303
      externalFilter->setCutoff(instCutoff);
    else
    ...
    {
      tmp  = ( externalOscillator != NULL ) ? -externalOscillator->getSample()   // ATEK303
                                            : -oscillator.getSample();         // the raw oscillator signal 
      tmp  = highpass1.getSample(tmp);        // pre-filter highpass
      tmp  = ( externalFilter != NULL ) ? externalFilter->getSample(tmp)   // ATEK303
                                        : filter.getSample(tmp);
    ...
    if( otaHeadroom > 0.0 )                                          // ATEK303
      tmp = otaHeadroom * tanh(tmp / otaHeadroom);                   // OTA BA662A
