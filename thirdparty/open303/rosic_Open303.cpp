...
{
  // El upstream configuraba todos los bloques, pero no actualizaba este miembro. El
  // oscilador interno recibe newSampleRate directamente y ocultaba el fallo; el gancho
  // externo usaba después el valor inicial de 44,1 kHz y quedaba +146 cents a 48 kHz.
  sampleRate = newSampleRate;                                                  // ATEK303
  ...
  if( externalFilter != NULL )                                                  // ATEK303
    externalFilter->setSampleRate(oversampling*newSampleRate);
}
...
void Open303::triggerNote(int noteNumber, bool hasAccent)
{
  ...
  if( idle )
  {
    if( externalOscillator != NULL )        // ATEK303
      externalOscillator->resetPhase();
    else
      oscillator.resetPhase();
    if( externalFilter != NULL ) externalFilter->reset(); else filter.reset();   // ATEK303
    highpass1.reset();
    highpass2.reset();
    allpass.reset();
    notch.reset();
  ...
  pitchSlewLimiter.setState(slideInPitchDomain ? log(oscFreq) : oscFreq);   // ATEK303
  mainEnv.trigger();
  ampEnv.noteOn(true);
  idle = false;
}
